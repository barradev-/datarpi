...
echo -n 'group: '
read group
socat -d -d PTY,link=/dev/ttyAMA0,raw,echo=0,crnl,user=$user,group=$group PTY,link=/dev/AMA1,raw,echo=0,crnl,user=$user,group=$group &
...
-I- Ventiladores encendidos." | tee /dev/ttyAMA0
